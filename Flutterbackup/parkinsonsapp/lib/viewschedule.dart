...
class ViewSchedule extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const ViewSchedulePage(title: 'Doctor Schedule'),
    );
  }
}

class ViewSchedulePage extends StatefulWidget {
  const ViewSchedulePage({super.key, required this.title});
...
class _ViewSchedulePageState extends State<ViewSchedulePage> {
  ...
  Future<void> view_notification() async {
    ...
    try {
      ...
      String url = '$urls/myapp/view_user_schedule/';
      ...
      for (int i = 0; i < arr.length; i++) {
        ...
        date.add(arr[i]['date']);
        fromtime.add(arr[i]['fromtime']);
        totime.add(arr[i]['totime']);
      }
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    _buildTimeSlot('From Time', fromtime_[index], Icons.access_time),
                    Container(
                      width: 1,
                      height: 50,
                      color: Colors.blueAccent.withOpacity(0.3),
                    ),
                    _buildTimeSlot('To Time', totime_[index], Icons.access_time),
                  ],
                ),
              ),
              SizedBox(height: 20),
              ...
              Container(
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    await _makeAppointment(id_[index]);
                  },
  ...
  Future<void> _makeAppointment(String scheduleId) async {
    try {
      SharedPreferences sh = await SharedPreferences.getInstance();
      String url = sh.getString('url').toString();
      String lid = sh.getString('lid').toString();

      final urls = Uri.parse('$url/myapp/and_user_take_appointment/');
